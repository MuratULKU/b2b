...
namespace Business.Concrete
{
    public class CategoryManager:ICategoryService
    {
        ...
        {
           return await _unitOfWork.Repository<Category>().Find(x => x.Parent == parentref);
         
        }

        public async Task<Category> Get(Guid id)
        {
           return await _unitOfWork.Repository<Category>().SingleOrDefaultAsync(x => x.Id == id);
        }

        public async Task<Category> GetByCode(string code)
        {
         return await _unitOfWork.Repository<Category>().SingleOrDefaultAsync(x => x.Code == code);
        
        }

        public async Task<Category> GetByRefNo(int refno)
        {
           return await _unitOfWork.Repository<Category>().SingleOrDefaultAsync(x=>x.LogicalRef == refno);  
        }
        ...
        {
         var result = await _unitOfWork.Repository<Category>().MaxAsync(x => x.LogicalRef);
            return result +1;
        }
